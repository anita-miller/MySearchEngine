    ...
    describe("enriches tickets") {
      it("enriches ticket documents") {
        ...
        enriched shouldBe List(Json.obj("id" := "123", "submitter_id" := 456, "submitter" := "Anita",
          "organization_id":= 700, "organization" := "Zendesk", "assignee_id" := 456,  "assignee" := "Anita"))
      }
      ...
      it("handles missing joins") {
        ...
        enriched shouldBe List(Json.obj("id" := "123", "submitter_id" := 789, "organization_id":= 100, "assignee_id" := 12435, "submitter" := "Could not find", "organization" := "Could not find", "assignee" := "Could not find"))
      }
    }
